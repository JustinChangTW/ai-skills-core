...
在使用者要建立、改版或檢核 skill 時使用。常見觸發像「skill review」「提高 skill 觸發率」「優化 SKILL.md」「補 trigger eval」。輸出可安裝、可測、邊界清楚的 skill；不適用於一次性 prompt 或單一 tool schema。

## 基本資料

- Skill ID：`allanyiin-skill-creator-advanced`
...
```text
$allanyiin-skill-creator-advanced 請依我的目標與現有資料完成任務，並列出需要我確認的事項。
```
...
- `references/`：按需求載入的參考資料（29 個檔案）
- `scripts/`：可重複執行的輔助工具（59 個檔案）
