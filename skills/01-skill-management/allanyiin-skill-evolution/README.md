...
適用於使用者要把失敗的 agent 執行、人工修正、regression、trigger 失敗或 skill library drift 轉成具體 skill 更新、eval、rollback、merge、split 或 retirement 決策。不適用於沒有失敗證據的全新 skill 建立；那類任務請用 skill-creator-advanced。

## 基本資料

- Skill ID：`allanyiin-skill-evolution`
...
```text
$allanyiin-skill-evolution 請依我的目標與現有資料完成任務，並列出需要我確認的事項。
```
