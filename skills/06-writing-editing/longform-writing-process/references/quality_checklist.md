...
這份 checklist 用來記錄 longform-writing-process 目前是否符合 `skill-creator-advanced` 的 readiness gate 規範。
...
- [ ] 有先建立寫作契約，再進入 Draft v0 與 review rounds。
- [ ] 每輪評審有角色差異、必修/可選修改與整合紀錄。
- [ ] 需要查核時有保留主要來源、限制與修訂脈絡。
