...
description: >
  ...
  Use when writing or supplementing a note that needs an embedded anatomy,
  classification, algorithm, or imaging figure; when the user asks to extract a
  figure; or when a note's TODO or REF points to a figure not yet embedded.
---
...
before running commands. The extraction pipeline is bundled under
`$SKILL_ROOT/scripts/project`; do not require a separate repository clone.

Install the core dependencies in `references/requirements.txt` before first
use. Treat source PDFs as read-only and write crops only to the output path
chosen by the user.
...
- ==Future changes MUST NOT re-introduce silent fallback (default→
  ...
  regression test suite (`$SKILL_ROOT/scripts/project/figures/test_contract.py`) runs after any gate
  edit.
...
```bash
python $SKILL_ROOT/scripts/project/figures/batch_remap.py --list
python $SKILL_ROOT/scripts/project/figures/batch_remap.py --book NAME --dry
python $SKILL_ROOT/scripts/project/figures/batch_remap.py --book NAME --apply
```
...
local vision model. See `$SKILL_ROOT/scripts/project/figures/CALIBRATION.md` for accumulated
per-book quirks.
...
> `$SKILL_ROOT/scripts/project/figures/batch_remap.py` degrades gracefully without it (falls
> back to the gate's own extraction path per figure).

## Files

Everything referenced above lives under `$SKILL_ROOT/scripts/project/figures/`:
