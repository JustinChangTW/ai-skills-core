...
在使用者要診斷 MCP HTTP 或 Streamable HTTP 端點、確認連線或列出工具時使用。常見觸發像「檢查這個 MCP 連結」「列出 tools」「看 /mcp 為什麼連不上」。輸出握手結果、工具清單與故障診斷；不負責實作或重構 server。

## 基本資料

- Skill ID：`mcp-http-diagnostics`
...
```text
$mcp-http-diagnostics 請依我的目標與現有資料完成任務，並列出需要我確認的事項。
```
