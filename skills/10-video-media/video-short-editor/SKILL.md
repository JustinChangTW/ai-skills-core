...
2. Check that the installed `video-use` workflow, `FFmpeg`, `ffprobe`, and
   ...
   is missing, hand off to `video-editing-setup` and state what must be
   verified first.
...
point. For environment gaps, use `video-editing-setup`; it must obtain
